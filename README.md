...
- 「ゲームの感想」から既存ゲームをブラッシュアップ。感想の自動保存、元ゲームを残した改善版の生成。
...
1. 右の「生成履歴・復旧」で生成完了したゲームを選び、「ゲームの感想」タブを開く。
2. よかった点や変えたい点を入力し、「この感想でブラッシュアップ」を押す。
3. 改善版の生成完了後に遊び、同じ欄から次の感想を送る。

感想の下書きはゲームごとに自動保存します。元のソースと補完企画を別フォルダへコピーして改善するため、履歴から以前の版へ戻れます。新規企画の入力は維持され、改善には選択したゲームの形式を使います。改善要望に関係する仕様は以前の明示指定より優先して変更し、無関係な仕様と未知の企画項目は引き継ぎます。

改善版には反映に使った感想と元フォルダを記録します。感想は最大10,000文字。失敗や取消でも元の下書きと途中の成果物を残し、自動再試行しません。元ゲーム配下への出力、シンボリックリンクを含むコピー、10,000ファイル・256MiBを超えるコピーは拒否します。隠しファイル・認証ファイル・秘密鍵・依存キャッシュなどの除外範囲は [SECURITY.md](SECURITY.md) を参照してください。
...
|baseline-*|改善前の企画・README・判断理由・完了レポート|
...
「生成完了」は終了コード、`turn.completed`、空でない必須ファイル、全項目の補完、新規生成での明示指定の維持、完了レポートのStatus=completedと阻害事項なしを確認した状態です。改善でも今回の補完企画と完了レポートを新しく要求し、元の未知項目の欠落を検査します。プレイテスト・コンパイルの合格を意味しません。ブラウザ形式は「ゲームを開く」から既定ブラウザで開けます。Unity/Avalonia形式はREADMEの手順に従って利用者が起動します。
